 ...
 *
 * @category  NFePHP
 * @package   NFePHP\NFCom\Common\Tools
...
use NFePHP\NFCom\Factories\Contingency;
use NFePHP\NFCom\Factories\ContingencyNFCom;
use NFePHP\NFCom\Factories\Header;
use NFePHP\NFCom\Factories\QRCode;
...
class Tools
{
     ...
     * @var string
     */
    protected $urlPortal = 'http://www.portalfiscal.inf.br/nfcom';
    /**
     * urlcUF
     * @var int
     */
    ...
    public function getTypeOfPersonFromCertificate(): string
    {
        ...
        $type = 'J';
        if (empty($cnpj)) {
            //n�o � CNPJ, ent�o verificar se � CPF
            $cpf = $this->certificate->getCpf();
            if (!empty($cpf)) {
                $type = 'F';
            } else {
                //n�o foi localizado nem CNPJ e nem CPF esse certificado n�o � us�vel
    ...
    /**
     * Set or get model of document NFCom = 62
     * @param int $model
    ...
    public function validKeyByUF(string $chave): string
    {
        ...
        if ($uf != UFList::getUFByCode((int)substr($chave, 0, 2))) {
            throw new \InvalidArgumentException(
                "A chave da NFCom indicada [$chave] n�o pertence a [$uf]."
            );
        }
        return $uf;
    }
    ...
    {
        if (empty($xml)) {
            throw new InvalidArgumentException('O argumento xml passado para ser assinado est� vazio.');
        }
        //remove all invalid strings
        $xml = Strings::clearXmlString($xml);
        if ($this->contingency->type !== '') {
            $xml = ContingencyNFCom::adjust($xml, $this->contingency);
        }
        $signed = Signer::sign(
            $this->certificate,
            $xml,
        ...
        $infNFComSupl = !empty($dom->getElementsByTagName('infNFComSupl')->item(0));
        if ($modelo == 62 && !$infNFComSupl) {
            $signed = $this->addQRCode($dom);
        }
        //exception will be throw if NFe is not valid
        $this->isValid($this->versao, $signed, 'nfcom');
        return $signed;
    }

    /**
     * Corrects NFCom fields when in contingency mode
     * @param string $xml NFe xml content
     */
    protected function correctNFComForContingencyMode(string $xml): string
    {
        return $this->contingency->type == '' ? $xml : $this->signNFCom($xml);
    }
        ...
        if (!empty($type)) {
            if ($type !== 'SVCRS' && $type !== 'SVCAN') {
                throw new RuntimeException(
                    "Esse modo de conting�ncia [$type] n�o possue webservice pr�prio, portanto n�o haver�o envios."
                );
            }
        }
    }
    ...
    protected function servico(string $service, string $uf, $tpAmb, bool $ignoreContingency = false): void
    {
        ...
        //NT 2024.002 1.00 Maio/2024, coment�rio P08 elemento cOrgao
        if ($uf === 'SVRS') {
            $this->urlcUF = 92;
        } else {
            $this->urlcUF = $this->getcUF($uf); //recupera��o do cUF
            if ($this->urlcUF > 91) {
                $this->urlcUF = $this->getcUF($this->config->siglaUF); //foi solicitado dado de SVCRS ou SVCAN
            }
        }
        $this->urlVersion = $stdServ->$service->version; //recupera��o da vers�o
        $this->urlService = $stdServ->$service->url; //recupera��o da url do servi�o
        $this->urlMethod = $stdServ->$service->method; //recupera��o do m�todo
        $this->urlOperation = $stdServ->$service->operation; //recupera��o da opera��o
    ...
    {
        $file = "wsnfcom_" . $this->versao . "_mod62.xml";
    ...
    protected function addQRCode(DOMDocument $dom): string
    {
        if (empty($this->config->CSC) || empty($this->config->CSCid)) {
            throw new \RuntimeException("O QRCode n�o pode ser criado pois faltam dados CSC e/ou CSCId");
        }
        $memmod = $this->modelo;
        $this->modelo = 62;
        ...
        $this->servico('NfcomConsultaQR', $uf, $tpAmb);
        $signed = QRCode::putQRTag(
            ...
            $this->urlVersion,
            $this->urlService,
            $this->getURIConsultaNFCom($uf, $tpAmb)
        );
        $this->modelo = $memmod;
     ...
     */
    protected function getURIConsultaNFCom(string $uf, string $tpAmb): string
    {
        $arr = json_decode(file_get_contents($this->pathwsfiles . 'uri_consulta_nfcom.json'), true);
    ...
    protected function checkModelFromXml(string $xml): void
    {
        ...
        $correct = 62;
        if (!$check) {
            throw new InvalidArgumentException('Voc� passou um XML de modelo incorreto. '
                . "Use o m�todo \$tools->model({$correct}), para selecionar o "
